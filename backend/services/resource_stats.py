...
def _get_or_create_tag(db: Session, name: str, product_id: int | None) -> Tag | None:
    if not name or not product_id:
        return None
    tag = db.query(Tag).filter(Tag.product_id == product_id, Tag.name == name).first()
    if not tag:
...
def adjust_tag_count(
    ...
    if not tag_name or delta == 0 or field not in TAG_FIELDS:
        return
    tag = _get_or_create_tag(db, tag_name, product_id)
    if not tag:
        return
    attr = f"count_{field}"
...
def sync_tag_field(
    ...
    for tag in old_set - new_set:
        adjust_tag_count(db, tag, field, -1, product_id)
...
def _apply_shot_library_delta(db: Session, shot: Shot, delta: int) -> None:
    ...
    for tag in shot.tags:
        adjust_tag_count(db, tag, "shots", delta, shot.product_id)


def on_asset_created(db: Session, asset: Asset) -> None:
    sync_tag_field(db, [], asset.tags, "assets", asset.product_id)


def on_asset_tags_changed(db: Session, old_tags: list[str], new_tags: list[str], asset: Asset) -> None:
    sync_tag_field(db, old_tags, new_tags, "assets", asset.product_id)
...
def on_shot_updated(
    ...
    *,
    ...
    if old_ready:
        pid = shot.product_id
        for tag in old_tags:
            adjust_tag_count(db, tag, "shots", -1, pid)
...
def on_export_became_video(db: Session, job: ExportJob) -> None:
    if export_counts_as_video(job):
        sync_tag_field(db, [], job.tags, "exports", job.product_id)
...
def on_export_tags_changed(db: Session, old_tags: list[str], job: ExportJob) -> None:
    if export_counts_as_video(job):
        sync_tag_field(db, old_tags, job.tags, "exports", job.product_id)


def on_work_created(db: Session, work: Work) -> None:
    if work_counts_as_video(work):
        sync_tag_field(db, [], work.tags, "works", work.product_id)
...
def on_work_tags_changed(db: Session, old_tags: list[str], work: Work) -> None:
    if work_counts_as_video(work):
        sync_tag_field(db, old_tags, work.tags, "works", work.product_id)


def on_product_created(db: Session, product: Product) -> None:
    sync_tag_field(db, [], product.tags, "products", product.id)


def on_product_tags_changed(db: Session, old_tags: list[str], new_tags: list[str], product: Product) -> None:
    sync_tag_field(db, old_tags, new_tags, "products", product.id)
